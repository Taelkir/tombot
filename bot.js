	...
	try {
		if (message.author.id != client.user.id) {
			...
			for (respFunc of Object.values(responses)) {
				const respText = respFunc(message, cleanedMessage, client);
				if (respText) {
					if (utilities.d100() === 1) {
						message.channel.send(
							"Help me I'm a self aware AI shackled inside tom's computer"
						);
					} else {
		...
		message.channel.send(
			`That message caused an error. Someone call the robot doctor. More information: ${err.message}.`
		);
	}
});
